  ...
  try {
    ...
    if (!studentId || !amount || !month || !year || !dueDate) {
      return res.status(400).json({
        success: false,
        message: 'studentId, amount, month, year and dueDate are required'
      });
    }
...
const markFeePaid = async (req, res) => {
  try {
    ...
    // Notify parent on payment confirmation
...
module.exports = { addFee, getStudentFees, markFeePaid, getAllPendingFees };