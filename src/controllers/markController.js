  ...
  try {
    ...
    const ops = marksData.map(({ studentId, marksObtained }) => ({
      updateOne: {
        ...
        update: {
          $set: {
            student: studentId, teacher: teacher._id, school: teacher.school,
            class: cls, section, subject, examName, examType,
            maxMarks, marksObtained, weightage, academicYear, status: 'draft',
          }
        },
        upsert: true,
      }
